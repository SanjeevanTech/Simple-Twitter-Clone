...
const Post = ({ post }) => {
	...
	const [isBookMarked, setIsBookMarked] = useState(authuser?.bookmarkedPosts?.includes(post._id));
	const { mutate: bookmarkPost, isPending: isBookmarking } = useMutation({
		mutationFn: async () => {
		  try {
			
			const res = await fetch(`${baseUrl}/api/posts/bookmark/${post._id}`, {
			  method: "POST",
			  credentials: "include",
			  headers: {
				"Content-Type": "application/json",
	  ...
	  const { mutate: repostPost, isPending: isReposting } = useMutation({
		mutationFn: async () => {
		  try {
			const res = await fetch(`${baseUrl}/api/posts/repost/${post._id}`, {
			  method: "POST",
			  credentials: "include",
			  headers: {
				"Content-Type": "application/json",
				...
				<div className='flex flex-col flex-1'>
					...
					<div className='flex justify-between mt-3'>
						<div className='flex gap-4 items-center w-2/3 justify-between'>
							...
							{isReposting ? (
            					<LoadingSpinner size="sm" />  // Show a spinner when reposting
									) : (
										<BiRepost 
										className={`w-6 h-6 ${isReposting ? 'text-slate-500' : 'text-slate-500 group-hover:text-green-500'}`} 
									  />
									)}
						...
						<div className='flex w-1/3 justify-end gap-2 items-center' onClick={handleBookMarkPost}>
								...
								{!isBookMarked && !isBookmarking && (
									<div className="group">
									<FaRegBookmark className="w-4 h-4 cursor-pointer group-hover:text-teal-500" />
								 </div>
								)}
								{isBookMarked && !isBookmarking && (
									<FaRegBookmark className="w-4 h-4  cursor-pointer text-teal-500" />
								)}
