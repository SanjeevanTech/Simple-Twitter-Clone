    ...
    try{
        ...
        if(isFollowing){
           ...
           await newNotification.save();
            ...
            res.status(200).json({message : "follow Succesfully"})

            
        }

    }catch(error){
        console.log(`error followunfollow controller : ${error}`)
        res.status(500).json({error : "Internal server Error"});
    }
}
...
export const getSuggestUsers=async(req,res)=>{
    try{
        ...
        console.log(`Error in getSuggest Controller ${error}`)
        res.status(500).json({error : "Internal Server error"})
    }
}
...
export const updateUser=async(req,res)=>{
    try{
        ...
        if((!newPassword && currentPassword) || (!currentPassword && newPassword)){
            return res.status(404).json({error : "please provide a both new password ,current password"})
        }
        ...
        if(newPassword && currentPassword){
            ...
            if(newPassword.length<6){
                return res.status(404).json({error : "Password must have 6 chars"})
            }
